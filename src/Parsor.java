...
public class Parsor {
     ...
     */
    public static Jeu parse(String fileName) {
        SAXBuilder builder = new SAXBuilder();
        try {
            File f = new File(fileName);
            Element xmlRoot = builder.build(f).getRootElement();

            int annee = Integer.decode(xmlRoot.getChildText("annee"));
            ...
            return new Jeu(annee, categories, shortDescription, dpublic, title,
    ...
    public static void main(String[] args) {
        Jeu test = parse("../resources/infos.xml");
        if (test != null) {
